...
- [x] **Backend reorganization** — Restructured Go backend to colocate game mode logic under `internal/gamemode/` (replacing `scheduler/`). Created service layer per game mode (americano, mexicano, timed) to consolidate orchestration. Slimmed `api/` to pure HTTP dispatch. All tests passing, zero logic changes.
