...
func NewRouter(s *store.Store, emailClient *email.Client, appURL, vapidPrivate, vapidPublic string, rateLimit RateLimitConfig) http.Handler {
	...
	r.Route("/api", func(r chi.Router) {
		...
		r.With(h.requireAuth).Patch("/auth/self_rating", h.updateSelfRating)
		r.With(h.requireAuth).Get("/auth/stats", h.stats)
		...
		r.With(h.requireAuth).Get("/clubs/{id}/events", h.getClubEvents)
		r.With(h.requireAuth).Get("/clubs/{id}/leaderboard", h.getClubLeaderboard)
		r.With(h.requireAuth).Patch("/clubs/{id}", h.updateClub)
		r.With(h.requireAuth).Delete("/clubs/{id}", h.deleteClub)
		...
		r.With(h.requireAuth).Patch("/clubs/{id}/members/{userID}", h.updateClubMemberRole)
		r.With(h.requireAuth).Delete("/clubs/{id}/members/{userID}", h.removeClubMember)
